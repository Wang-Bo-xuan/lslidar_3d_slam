...
public:
  ...
  void addPointCloud(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& point_cloud);

  void setMapUpdateDistance(float distance)
  {
    map_update_distance_ = distance;
  }

  void setMapUpdateAngle(float angle)
  {
    map_update_angle_ = angle;
  }

  void setSubmapSize(int size)
  {
    submap_size_ = size;
  }
...
private:
  ...
  std::vector<pcl::PointCloud<pcl::PointXYZRGB>::Ptr> submap_;
